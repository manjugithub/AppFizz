...
//static NSString *const LQSCurrentUserID;
//static NSString *const LQSParticipantUserID;
// extern NSString *const LQSParticipant2UserID;
NSString *const LQSInitialMessageText;
NSString *const LQSCategoryIdentifier;
...
- (void)viewDidLoad
{
    ...
    self.LQSCurrentUserID = [BUWebServicesManager sharedManager].userID;
...
- (void)fetchLayerConversation
{
    ...
    if (!self.conversation) {
        ...
        self.conversation = [self.layerClient newConversationWithParticipants:[NSSet setWithArray:@[ _LQSParticipantUserID ]] options:nil error:&error];
        if (!self.conversation) {
...
- (void)configureCell:(LQSChatMessageCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([message.sender.userID isEqualToString:_LQSCurrentUserID]) {
        switch ([message recipientStatusForUserID:_LQSParticipantUserID]) {
